...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-black/80 backdrop-blur-sm transition-all duration-500 data-[state=closed]:opacity-0 data-[state=open]:opacity-100 data-[state=closed]:backdrop-blur-none",
      className
    )}
    {...props}
    ref={ref}
...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background shadow-lg transition-all duration-500 ease-out",
  {
    variants: {
      side: {
        ...
        right:
          "inset-y-0 right-0 h-full w-full border-l data-[state=closed]:translate-x-full data-[state=open]:translate-x-0",
        "right-mobile":
          "inset-y-0 right-0 h-full w-full border-l data-[state=closed]:translate-x-full data-[state=open]:translate-x-0",
      },
    },
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {/* Only show default close button if it's not a mobile navbar (we have custom close) */}
      {side !== "right-mobile" && (
